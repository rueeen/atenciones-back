...
class PsychopedagogyModuleAccessMixin(LoginRequiredMixin):
    def dispatch(self, request, *args, **kwargs):
        if not can_access_psychopedagogy_module(request.user):
            messages.error(request, 'No tiene permisos para acceder al módulo psicopedagógico.')
...
class PsychopedagogyRecordCreateView(PsychopedagogyModuleAccessMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, 'Ficha psicopedagógica creada correctamente.')
        return super().form_valid(form)


class PsychopedagogyLogEntryCreateView(PsychopedagogyModuleAccessMixin, View):
    def post(self, request, pk):
        record = get_object_or_404(visible_psychopedagogy_records_for(request.user), pk=pk)
        if not can_edit_psychopedagogy_record(request.user, record):
            messages.error(request, 'No tiene permisos para agregar bitácora en esta ficha.')
        ...
        if form.is_valid():
            ...
            entry.save()
            messages.success(request, 'Entrada de bitácora agregada correctamente.')
        else:
            messages.error(request, 'No se pudo agregar la bitácora. Revise los datos ingresados.')
        return redirect('psychopedagogy:detail', pk=pk)
...
class PsychopedagogyAttachmentCreateView(PsychopedagogyModuleAccessMixin, View):
    def post(self, request, pk):
        record = get_object_or_404(visible_psychopedagogy_records_for(request.user), pk=pk)
        if not can_edit_psychopedagogy_record(request.user, record):
            messages.error(request, 'No tiene permisos para adjuntar archivos en esta ficha.')
            return redirect('psychopedagogy:detail', pk=pk)
        ...
        if form.is_valid():
            ...
            attachment.save()
            messages.success(self.request, 'Archivo adjunto cargado correctamente.')
        else:
            messages.error(self.request, 'No se pudo cargar el adjunto. Revise formato y tamaño de archivo.')
        return redirect('psychopedagogy:detail', pk=pk)
